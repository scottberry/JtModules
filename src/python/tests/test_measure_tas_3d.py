...
class test_measure_tas_3d(unittest.TestCase):
    
    def test_measure_tas_3d_actual_data(self): 
        ...
        measurements = measure_tas_3d(label_image, intensity_image, plot=False)
        measurements_tas = measurements['measurements']

        intensity_image = np.zeros((20, 20), dtype=np.uint16)
        intensity_image[13:15, 13:17] = 230
        intensity_image = np.uint16(intensity_image, dtype=np.uint16)
        
        label_image = np.zeros((20, 20), dtype=np.double)
        label_image[13:15, 13:17] = 1
        label_image = label(label_image > 0)
        
        measurements1 = measure_tas_3d(label_image, intensity_image,plot=False)
        measurements_tas1 = measurements1['measurements']

        assert (measurements_tas.shape[1] == measurements_tas1.shape[1]
                                    ), "measurements of the images are correct"
        

    def test_measure_tas_3d_fake_data(self): 
        ...
        measurements2 = measure_tas_3d( label_image2, intensity_image2, plot=False)
        measurements_tas2 = measurements2['measurements']

#        assert_almost_equal(measurements_hu1, measurements_hu2, decimal=1)
        
        assert (measurements_tas1.shape[1] == measurements_tas2.shape[1]
                                    ), "measurements of the images are correct"
        
    
    def test_tas_3d(self):
        
        np.random.seed(22)
        intensity_image = np.random.rand(512, 512, 8)
        intensity_image = np.uint16(intensity_image * 255, dtype=np.uint16)
        label_image2 = label(intensity_image > 130) 
        
        measurements2 = measure_tas_3d(label_image2, intensity_image, plot=False)
        measurements2 = measurements2['measurements']
        
        # assert np.abs(np.sum(measurements2[1])-6) < 0.0001
         
        for i in range(0,7):
            assert (measurements2[i].shape[1] == 54
                                    ), "measurements of the images are correct"
