...
from measure_morphology_3d import *
import numpy as np
import cv2
...
class test_measure_morphology_3d(unittest.TestCase):
    
    def test_measure_morphology_3d_actual_data(self): 
...
#        intensity_image = cv2.imread(temp_input_file, cv2.IMREAD_UNCHANGED) 
        ...
        label_image = cv2.imread(temp_output_file, cv2.IMREAD_UNCHANGED) 
        label_image = label(label_image > 0)
       
        measurements = measure_morphology_3d(label_image, plot=False)
        measurements_morphology = measurements['measurements']

        morphology_image = np.zeros((20, 20), dtype=np.uint16)
        morphology_image[13:15, 13:17] = 230
        
        label_image = np.zeros((20, 20), dtype=np.double)
        label_image[13:15, 13:17] = 1
        label_image = label(label_image > 0)
        
        measurements1 = measure_morphology_3d(label_image, plot=False)
        measurements_morphology1 = measurements1['measurements']

        assert (measurements_morphology.shape[1] == measurements_morphology1.shape[1]
                                    ), "measurements of the images are correct"
        

    def test_measure_morphology_3d_fake_data(self): 
        ...
        morphology_image2 = np.zeros((20, 20), dtype=np.uint16)
        morphology_image2[11:13, 11:13] = 150
        
        label_image2 = np.zeros((20, 20), dtype=np.double)
        label_image2[11:13, 11:13] = 1
        label_image2 = label(label_image2 >0)
        
        measurements2 = measure_morphology_3d(label_image2, plot=False)
        measurements_morphology2 = measurements2['measurements']

#        assert_almost_equal(measurements_hu1, measurements_hu2, decimal=1)
        
        assert (measurements_morphology1.shape[1] == measurements_morphology2.shape[1]
                                    ), "measurements of the images are correct"
