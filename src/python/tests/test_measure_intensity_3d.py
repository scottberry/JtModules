...
from measure_intensity_3d import *
import numpy as np
import cv2
...
class test_measure_intensity_3d(unittest.TestCase):
    
    def test_measure_intensity_3d_actual_data(self): 
        ...
        measurements = measure_intensity_3d(label_image, intensity_image, plot=False)
        measurements_intensity = measurements['measurements']
        ...
        measurements1 = measure_intensity_3d(label_image, intensity_image, plot=False)
        measurements_intensity1 = measurements1['measurements']

        assert (measurements_intensity.shape[1] == measurements_intensity1.shape[1]
                                    ), "measurements of the images are correct"
        

    def test_measure_intensity_3d_fake_data(self): 
        ...
        measurements2 = measure_intensity_3d(label_image2,intensity_image2, plot=False)
        measurements_intensity2 = measurements2['measurements']

#        assert_almost_equal(measurements_hu1, measurements_hu2, decimal=1)
        
        assert (measurements_intensity1.shape[1] == measurements_intensity2.shape[1]
                                    ), "measurements of the images are correct"
