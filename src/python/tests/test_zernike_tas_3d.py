...
from measure_zernike_3d import *
import numpy as np
import cv2
...
class test_measure_zernike_3d(unittest.TestCase):
    
    def test_measure_zernike_3d_actual_data(self): 
        ...
        measurements = measure_zernike_3d(label_image, plot=False)
        measurements_zernike = measurements['measurements']
        ...
        measurements1 = measure_zernike_3d( label_image, plot=False)
        measurements_zernike1 = measurements1['measurements']

        assert (measurements_zernike.shape[1] == measurements_zernike1.shape[1]
                                    ), "measurements of the images are correct"
        

    def test_measure_zernike_3d_fake_data(self): 
        ...
        measurements2 = measure_zernike_3d( label_image2, plot=False)
        measurements_zernike2 = measurements2['measurements']

#        assert_almost_equal(measurements_hu1, measurements_hu2, decimal=1)
        
        assert (measurements_zernike1.shape[1] == measurements_zernike2.shape[1]
                                    ), "measurements of the images are correct"
        
    
    def test_zernike_3d(self):
        ...
        for i in range(0,7):
            assert (measurements2[i].shape[1] == 49
                                    ), "measurements of the images are correct"
