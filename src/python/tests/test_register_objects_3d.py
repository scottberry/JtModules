...
from register_objects_3d import *
import numpy as np
import cv2
import os
...
class test_register_objects_3d(unittest.TestCase):
    
    def test_register_objects_3d_actual_data(self): 
        ...
        registered = register_objects_3d(label_image)
        registered_label_image = np.int32(registered['objects'])

        assert (len(np.unique(label_image))-1 == 
                len(np.unique(registered_label_image))-1
    ...
    def test_register_objects_3d_fake_data(self): 
        
        A = np.zeros((128,128), np.int)
        L1 = label_mask_3d(A)
        L = L1['label_image']
        registered = register_objects_3d(L)
        registered_label_image = np.int32(registered['objects'])
        assert not registered_label_image.max()
        ...
        registered = register_objects_3d(L)
        registered_label_image = np.int32(registered['objects'])
        assert registered_label_image.max() == 2
        assert np.sum(registered_label_image > 0) == (40*40 + 3*3)
        assert np.all((registered_label_image > 0) == (A > 0) )
        assert set(registered_label_image.ravel()) == set([0,1,2])
    ...
    def test_all_ones(self):
        ...
        registered = register_objects_3d(L)
        registered_label_image = np.int32(registered['objects'])
        assert np.all(registered_label_image == 1)
    ...
    def test_random(self):
        ...
        registered = register_objects_3d(L)
        registered_label_image = np.int32(registered['objects'])
        assert len(set(registered_label_image.ravel())) == (len(
                                        np.unique(registered_label_image))-1+1)
        assert registered_label_image.max() == len(
                                        np.unique(registered_label_image))-1
